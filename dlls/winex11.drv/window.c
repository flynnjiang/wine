...
static void x11_dump_window_styles( HWND hwnd )
{
    DWORD style = NtUserGetWindowLongW(hwnd, GWL_STYLE);
    DWORD exstyle = NtUserGetWindowLongW(hwnd, GWL_EXSTYLE);

    TRACE( "hwnd %p style:", hwnd );
...
static BOOL is_window_managed( HWND hwnd, UINT swp_flags, const RECT *window_rect )
{
    ...
    if (!(swp_flags & (SWP_HIDEWINDOW))) return TRUE;
...
static void set_style_hints( struct x11drv_win_data *data, DWORD style, DWORD ex_style )
{
	...
	TRACE("hwnd %p whole %lx XChangeProperty _NET_WM_WINDOW_TYPE_DIALOG\n", data->hwnd, data->whole_window);
        window_type = x11drv_atom(_NET_WM_WINDOW_TYPE_DIALOG);
    } else {
	TRACE("hwnd %p whole %lx XChangeProperty _NET_WM_WINDOW_TYPE_NORMAL\n", data->hwnd, data->whole_window);
...
static void set_wm_hints( struct x11drv_win_data *data )
{
	...
	TRACE("hwnd %p whole/client %lx/%lx managed %u\n", data->hwnd, data->whole_window, data->client_window, data->managed);
	x11_dump_window_styles(data->hwnd);
    }
...
void update_net_wm_states( struct x11drv_win_data *data )
{
    ...
    if (!data->managed) { TRACE("hwnd %p whole_window %lx return\n", data->hwnd, data->whole_window); return;}
        ...
        if (type == XA_ATOM && format == 32)
        {
            for (i = 0; i < count; i++)
            {
                ...
                for (j=0; j < NB_NET_WM_STATES; j++)
                {
                    ...
                    {
			TRACE("hwnd %p whole %lx net_wm_state %u on\n", data->hwnd, data->whole_window, j);
                        new_state |= 1 << j;
                    }
                }
            }
...
static void create_whole_window( struct x11drv_win_data *data )
{
    ...
    if (!data->managed && is_window_managed( data->hwnd, 0, &data->window_rect ))
    {
    ...
    TRACE("win %p/%lx create new whole window\n", data->hwnd, data->whole_window);
                              ...
                              struct window_surface *surface )
{
    ...
    TRACE("hwnd %p whole/client %lx/%lx managed %u mapped %u\n", hwnd, data->whole_window, data->client_window, data->managed, data->mapped);

    x11_dump_window_styles( hwnd );
